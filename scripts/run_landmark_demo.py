...
원본: https://github.com/JIWOO1127/machine-vision1/blob/feature/seojiwoo-detection/seojiwoo/core/demo_live.py
      ...
      Locator/OcrVerifier/Navigator를 불러오고 기본 모델 경로를 이 프로젝트의
      models/yolo/에 맞췄다. 격자 지도 쪽의 "현재 위치" 판정은
      pipeline/grid_tracker.py가 별도로 담당하며(Navigator 상태 머신을 쓰지
      않음), --nav는 이 시연 스크립트 전용이다.
...
# 2모델 구성(2026-09-18): landmark_best.pt(표지판·문, 5클래스) + logo_best.pt(로고 전용).
# 6클래스로 합쳐 재학습하면 표지판 원거리 탐지가 무너지는 문제가 있어 분리함
# (seojiwoo/README.md "자주 막히는 곳" 참고).
DEFAULT_WEIGHTS = PROJECT_ROOT / "models" / "yolo" / "landmark_best.pt"
DEFAULT_LOGO_WEIGHTS = PROJECT_ROOT / "models" / "yolo" / "logo_best.pt"

ap = argparse.ArgumentParser()
ap.add_argument('--weights', default=str(DEFAULT_WEIGHTS), help='주 모델 (표지판·문): 5클래스 landmark_best.pt')
ap.add_argument('--logo_weights', default=str(DEFAULT_LOGO_WEIGHTS), help='로고 전용 모델 (지정 안 하면 --weights 하나로만 인식)')
...
ap.add_argument('--nav', action='store_true', help='시나리오 내비게이션 (로고→벽→4→3→2→뒷문, 방향 안내 포함). --route보다 우선함')
ap.add_argument('--wall_m', type=float, default=1.2, help='--nav 전용: 로고 벽에 도달했다고 볼 거리(m)')
ap.add_argument('--merge_signs', action='store_true', help='2/4 표지판을 하나로 합쳐 투표, 숫자는 OCR (--ocr 필요, --nav는 항상 켜짐)')
...
nav = None
if args.nav:
    from classroom_locator.landmark_locator import Navigator
