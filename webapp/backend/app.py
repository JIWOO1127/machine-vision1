...
frontend/(machine-vision1 저장소의 React 앱을 그대로 가져온 것)가 기대하는
API 모양(https://github.com/JIWOO1127/machine-vision1/tree/main/backend의
app.py 참고)을 최대한 따르되, 실제 판정은 vision_bridge.VisionBridge를 통해
우리 classroom_locator 파이프라인(SeojiwooGridPositionTracker)이 처리한다.

사용법:
    conda activate classroom-locator
    cd webapp/backend
    python app.py
    # 프론트 개발 서버는 별도로: cd ../frontend && npm install && npm run dev
    #   (vite.config.js가 이미 /api -> http://127.0.0.1:5000 프록시를 설정해둠)
"""
...
def create_app() -> Flask:
    ...
    def analyze():
        ...
        except Exception as exc:  # noqa: BLE001 - 사용자에게 원인 그대로 보여줌
    ...
    def analyze_video():
        # TODO: 영상 업로드 분석(서버에서 프레임별로 돌려서 오버레이 mp4 + 타임라인
        # 반환)은 아직 이식 안 함 - 사진/실시간 카메라 경로만 지원.
        # docs/setup_log.md의 webapp 관련 항목 참고.
        return jsonify(
    ...
    def frontend_index():
        if not FRONTEND_DIST.exists():
            return jsonify(error="frontend/dist가 없습니다. webapp/frontend에서 npm run build를 실행하세요."), 503
